...
def loadData(args):
    ...
    if not os.path.exists(data_url):
        ...
        shutil.copy(
            os.path.join(args.main_dir, dir_name, "ontology.json"), args.main_dir
        )
        shutil.copy(
            os.path.join(args.main_dir, dir_name, "valListFile.json"), args.main_dir
        )
        shutil.copy(
            os.path.join(args.main_dir, dir_name, "testListFile.json"), args.main_dir
        )
        shutil.copy(
            os.path.join(args.main_dir, dir_name, "dialogue_acts.json"), args.main_dir
        )
...
def divideData(data, args):
    ...
    copyfile(
        os.path.join(args.main_dir, "ontology.json"),
        os.path.join(args.target_path, "ontology.json"),
    )
    ...
    for dialogue_name in data:
        ...
        if dial:
            ...
            for turn_i, turn in enumerate(dial):
                ...
                turn_exmaple["system"] = (
                    dial[turn_i - 1]["sys"] if turn_i > 0 else "none"
                )
